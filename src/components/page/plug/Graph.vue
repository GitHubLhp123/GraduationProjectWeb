<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
    <div>
        ...
        <el-button type="success" @click="removeAll">清空组件</el-button>
            ...
            <el-tab-pane label="列表" name="second">
                <el-tabs v-model="activePlugTabName">
                    <el-tab-pane v-for="item in plugDataList" :label="item.label" :key="item.id">
                        ...
                        <parameters v-if="item.meta.name==='parameters'" v-model="item.data"
                                    v-bind:data="item.data"></parameters>
                        <sftp-to-sftp v-if="item.meta.name==='SftpToSftp'" v-model="item.data"
                                      v-bind:data="item.data"></sftp-to-sftp>
                        <oss-to-sftp v-if="item.meta.name==='OssToSftp'" v-model="item.data"
                                     v-bind:data="item.data"></oss-to-sftp>

                        <mongodb-reader v-if="item.meta.name==='mongodbReader'" v-model="item.data"
                                         v-bind:data="item.data"></mongodb-reader>
                        <Cmd v-if="item.meta.name==='Cmd'" v-model="item.data"
                                        v-bind:data="item.data"></Cmd>
                    </el-tab-pane>
...
<script>
    ...
    import SftpToSftp from './SftpToSftp';
    import OssToSftp from './OssToSftp';
    import Cmd from './Cmd';
    const drawerType = {
        node: 0,
        link: 1
    ...
    export default {
        ...
        components: {
            mysqlReader, mysqlWriter, parameters, SftpToSftp, OssToSftp, MongodbReader,Cmd
        },
        created() {
        ...
        methods: {
            ...
            saveGraph() {
                ...
                if (flag) {
                    ...
                    let toJson = this.$refs.superFlow.toJSON();
                    let linkListData = toJson.linkList;
                    for(let i=0;i<this.plugDataList.length;i++){
                        if(this.plugDataList[i].meta.name==="mongodbReader"){
                            this.plugDataList[i].data.column=JSON.stringify(this.plugDataList[i].data.column);
                            break;
                        }
            ...
            removeAll() {
                ...
                this.plugDataList = [];
                this.$refs.superFlow.linkList = [];
                this.$refs.superFlow.nodeList = [];
            },
